<x-app-layout>
    <x-slot name="header">
        <h2 class="topbar-title">Edit Produk</h2>
            ...
            <div class="card-dark-body">
                ...
                <form action="{{ route('products.update', $product) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <input type="text" name="kode_produk" value="{{ old('kode_produk', $product->kode_produk) }}" class="input-dark">
                    </div>

                    <div style="margin-bottom:16px;">
                        <label class="label-dark">Nama Produk <span style="color:#EF4444;">*</span></label>
                        <input type="text" name="nama_produk" value="{{ old('nama_produk', $product->nama_produk) }}" class="input-dark">
                    </div>
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <select name="kategori" class="input-dark">
                            ...
                            <option value="Snack"   {{ old('kategori', $product->kategori) == 'Snack'   ? 'selected' : '' }}>Snack</option>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px; margin-bottom:16px;">
                        <div>
                            ...
                            <input type="number" name="harga" value="{{ old('harga', $product->harga) }}" min="0" class="input-dark">
                        </div>
                        <div>
                            <label class="label-dark">Stok <span style="color:#EF4444;">*</span></label>
                            <input type="number" name="stok" value="{{ old('stok', $product->stok) }}" min="0" class="input-dark">
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom:16px;">
                        ...
                        <textarea name="deskripsi" rows="3" class="input-dark" placeholder="Contoh: Sate ayam dengan bumbu kacang pedas khas Taichan...">{{ old('deskripsi', $product->deskripsi) }}</textarea>
                    </div>
                    <div style="margin-bottom:24px;">
                        ...
                        <div style="display:flex; align-items:center; gap:16px;">
                            <div id="preview-container" style="width:80px; height:80px; border-radius:10px; border:1.5px dashed #E5E7EB; background:#FAF7F2; display:flex; align-items:center; justify-content:center; overflow:hidden; flex-shrink:0;">
                                @if($product->gambar)
                                    <img id="preview-img" src="{{ asset('storage/' . $product->gambar) }}" alt="" style="width:100%; height:100%; object-fit:cover;">
                                @else
                            ...
                            <div>
                                ...
                                <button type="button" onclick="document.getElementById('input-gambar').click()" class="btn-ghost">
                                    {{ $product->gambar ? 'Ganti Foto' : 'Pilih Foto' }}
                                </button>
